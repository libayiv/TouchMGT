        ...
        colModel: [			
			...
			{ label: '操作', name: 'operation', width: 80, formatter: function(value, options, row){
				if(hasPermission == null){
					$.ajax({
		                type: "GET",
		                url: baseURL + "sys/permitted/hasPermission?permission=sys:message:save",
				...
				if(hasPermission){
					...
					'<a class="btn btn-primary" onclick="vm.showDetail(\'' + row.id + '\')"><i class="fa fa-pencil-square-o"></i>&nbsp;查看</a>' ;
				}
				return '';
			} },
        ],
        ...
        getMessage: function(pid){
            ...
            $.get(baseURL + "sys/message/info/"+pid, function(r){
                ...
                if(r.message.coversrc != null && r.message.coversrc != ''){
                	...
                	$("#message_img").attr("src", localStorage.fileUrlPath + r.message.coversrc);
                } else {
